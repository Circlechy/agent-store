...
class SkillLoader:
    ...
    负责从多个来源发现和加载 skills：
    1. 本地 skills: ~/.jiuwen/skills/
    2. 项目 skills: <project>/.jiuwen/skills/
    ...
    def project_skills_dir(self) -> Path:
        """项目 skills 目录"""
        return self.project_root / ".jiuwen" / "skills"

    @property
    def installed_skills_dir(self) -> Path:
        """已安装 skills 目录（从 marketplace 安装）"""
        return Path.home() / ".jiuwen" / "plugins" / "cache"
    ...
    def _discover_installed_skills(self) -> List[Skill]:
        ...
        目录结构：
        ~/.jiuwen/plugins/cache/<marketplace>/<plugin>/<skill>/SKILL.md
