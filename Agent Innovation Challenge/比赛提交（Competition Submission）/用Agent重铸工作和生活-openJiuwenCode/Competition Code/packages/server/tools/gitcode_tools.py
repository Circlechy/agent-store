...
1. 同仓库 PR（不需要 head_owner）:
   owner: "openJiuwen", repo: "agent-core", head: "feature-branch", base: "develop"

2. 从 fork 创建 PR（推荐使用 head_owner）:
   owner: "openJiuwen", repo: "agent-core", head: "fix-bug", base: "develop", head_owner: "SnapeK"
   -> 自动转换为 head: "SnapeK:fix-bug"

3. 手动指定完整 head 格式:
   owner: "openJiuwen", repo: "agent-core", head: "SnapeK:fix-bug", base: "develop"
"""
        self.params = [
...
示例:
- owner: "openjiuwen", repo: "agent-core", number: "I123ABC"
"""
        self.params = [
            Param(name="owner", description="仓库所属空间地址", param_type="string", required=True),
            Param(name="repo", description="仓库路径", param_type="string", required=True),
