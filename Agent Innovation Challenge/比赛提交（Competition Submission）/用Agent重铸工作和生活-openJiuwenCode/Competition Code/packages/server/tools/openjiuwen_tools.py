"""
openjiuwen SDK 兼容的工具实现

基于 openjiuwen agent-core SDK 的 Tool 接口实现工具集
"""
...
"""
    ...
    def _is_plan_file(self, file_path: str) -> bool:
        """检查是否是 plan 文件（~/.jiuwen/plans/ 目录下的文件）"""
        if not file_path:
            return False
        # 规范化路径
        normalized_path = os.path.normpath(os.path.expanduser(file_path))
        plans_dir = os.path.normpath(os.path.expanduser("~/.jiuwen/plans"))
        # 检查是否在 plans 目录下
    ...
    async def ainvoke(self, inputs: dict, **kwargs) -> str:
        ...
        if self.mode_manager and self.mode_manager.get_current_mode() != AgentMode.BUILD:
            # 允许编辑 ~/.jiuwen/plans/ 目录下的 plan 文件
...
- 有超时限制（默认 30 秒）
- 在 PLAN 模式下允许写入 ~/.jiuwen/plans/ 目录下的 plan 文件
...
"""
    ...
    def _is_plan_file_operation(self, command: str) -> bool:
        """检查命令是否是对 plan 文件的操作（~/.jiuwen/plans/ 目录下的文件）"""
        if not command:
            return False
        # 获取 plans 目录路径
        plans_dir = os.path.expanduser("~/.jiuwen/plans")
        # 检查命令是否涉及 plans 目录
        return plans_dir in command or ".jiuwen/plans" in command
...
def create_all_tools(
    ...
    """
    # 导入 TodoWriteTool
    from .todo_tools import create_todo_tool
    # 导入浏览器工具
    ...
    tools = [
        ...
        create_ask_user_question_tool(mode_manager),  # AskUserQuestion 工具
        create_task_tool(
...
def create_tools_for_mode(
    ...
    if mode == AgentMode.BUILD:
        ...
        # PLAN 模式：只读工具 + TodoWrite + 只读浏览器工具 + Plan 工具 + WebFetch + AskUserQuestion + Task(受限)
        plan_mode_tools = [
            "read_file", "grep", "glob", "ls", "bash", "todo_write",
        ...
        # REVIEW 模式：只读工具 + TodoWrite + 只读浏览器工具 + Task(受限) + bash(用于 gh 等只读命令)
        read_only_tools = [
            "read_file", "grep", "glob", "ls", "todo_write", "bash",
