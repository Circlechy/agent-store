...
# os.environ["GOOGLE_CSE_ID"] = "909d04eb4f8ac40dd"
# os.environ["SEARCH_ENGINE_ID"] = "909d04eb4f8ac40dd"
# os.environ["GOOGLE_API_KEY"] = "AIzaSyAuTnHU9b4RQAzTwTmfPgDCQvlgwqKyFKw"
...
llm = OpenRouterLLM(api_key=os.getenv("OPENAI_API_KEY"), api_base=os.getenv("OPENAI_API_BASE"), model_name="gpt-4o-mini")
...
@retry(stop=stop_after_attempt(5), wait=wait_fixed(1), retry=retry_if_exception_type(Exception), reraise=True)
async def get_new_response(messages):
    json_schema = {
        "name": "my_schema",
        "strict": True,
        "schema": {
...
#     input_token = usage.prompt_tokens
#     output_token = usage.completion_tokens
#     total_cost = (15*input_token + 60*output_token)/100
#     # print(f"prompt token: {input_token}")
#     # print(f"completion token: {output_token}")
#     # print(f"total cost: {total_cost}")
#     return input_token, output_token, total_cost

def self_consistency_ensemble(problem, solutions):
    answer_mapping = {}
    solution_text = ""
    ...
    letter = get_response(messages)
    letter = letter.strip().upper()
    return solutions[answer_mapping[letter]]
...
# def get_response_on_tools(prompt, llm, file_path):
#     client = openai.OpenAI(api_key="sk-None-gdFuyMuBatz9mUEvmrxcT3BlbkFJ5opqc2zjXKFQqjVxgQCE")

#     # browser_toolkit_schema = browser_toolkit.get_tools()[0].openai_tool_schema
#     # browser_toolkit_schema['function']['description'] = 'A powerful toolkit which can search for online information and simulate the browser interaction to\nsolve the task which needs multi-step actions.'

#     sys_prompt = "You can answer questions directly or automatically select the appropriate tool when needed. Just give concise and accurate answer. Do not add any superfluous contents. If the chosen tool is \"browse_url\", parameter \"start_url\" should be usually \"https://www.google.com/\", unless website specified."
#     messages = [
...
#                 # content = image_toolkit.ask_question_about_image(args["image_path"], args["question"], args["sys_prompt"])
...
#                 # _, content = document_tookit.extract_document_content(args["document_path"], args["query"])
...
#         )
#         print(response2)
#         ans = response2.choices[0].message.content
#     else:
#         print("No tools")
#         ans = msg.content
#     return str(ans)
...
def get_dataset_info(data: str):
    ...
    content, rest = rest.strip().split("Role Description:")
    role_description_llm = rest.strip().split("Chosen LLM:")
    role_description = role_description_llm[0]
    if len(role_description_llm) == 2:
        llm = role_description_llm[1]
    else:
        llm = "gpt"
    return content.strip(), role_description.strip(), llm.strip()
...
def draw_picture(G: nx.DiGraph, wf_num):
    plt.figure(wf_num)
    ns = [n for n in G.nodes]
    pos = nx.spring_layout(G, seed=42, k=0.5)
    nx.draw_networkx_nodes(G, pos, nodelist=ns, node_color="skyblue", node_size=80)
    es = [(u, v) for u, v in G.edges]
    nx.draw_networkx_edges(G, pos, edgelist=es, edge_color="gray", width=1, alpha=0.5)
    nx.draw_networkx_labels(G, pos, font_size=6, font_weight="bold")
    
    plt.title("MANGO")
    plt.axis("off")
    os.makedirs("graph", exist_ok=True)
    jpg_path = f"graph/test{wf_num}.jpg"
...
def build_graph(f, tids, workflows, benchmark_selected, threshold=0.8):
    ...
    if benchmark_selected == "humaneval" or benchmark_selected == "mbpp":
        ...
        G.nodes[1]['role_description'] = "Answer Giver, the assistant gives the answer based on history messages. Please notice the subtasks and their answers mentioned earlier."
        system_prompt = "You will give the final answer of a reasoning question without any thought process and title."
    node_to_rd_vecs[1] = model_embedding.encode(G.nodes[1]['role_description'])
    node_to_content_vecs[1] = model_embedding.encode("Give the final answer based on the given information.")
    system_prompt = tg.Variable(system_prompt, requires_grad=True, role_description="structured system prompt to a somewhat capable language model that specifies the behavior and strategies for the QA task.")
    G.nodes[1]['updated_system_prompt'] = system_prompt
    ...
    for tid, workflow in zip(tids, workflows):
        ...
        for i, subtask in enumerate(subtasks):
            ...
            subtask, role_description, _  = get_dataset_info(subtask)
            role_description = role_description.replace("|", ",")

            # Extract the word and compute the embedding
            abstract, subtask = subtask.split("|", 1)

            subtask_embedding = model_embedding.encode(abstract)
            ...
            # for node, vec in node_to_content_vecs.items():
            #     if (i != 0 and node not in current_node_path and node not in starting_nodes) or (i == 0 and node in starting_nodes):
            #         content_similarity = cosine_similarity([torch.tensor(subtask_embedding)], [torch.tensor(vec)])[0][0]
            ...
            #         if content_similarity >= threshold and content_similarity > max_similarity:
            ...
            for node in node_to_content_vecs:
                ...
                if node not in current_node_path:
                    content_similarity = cosine_similarity([torch.tensor(subtask_embedding)], [torch.tensor(node_to_content_vecs[node])])[0][0]
                    rd_similarity = cosine_similarity([torch.tensor(rd_embedding)], [torch.tensor(node_to_rd_vecs[node])])[0][0]
                    similarity = (content_similarity + rd_similarity) / 2
                    # similarity = rd_similarity
            ...
            if max_similarity_node != -1:
                ...
                G.nodes[max_similarity_node]['subtasks'].append(subtask) # TODO: 记录该node曾经解决过的subtask内容，可提供给大模型参考，将概括与内容分离
                ...
                node_to_content_vecs[max_similarity_node] = (node_to_content_vecs[max_similarity_node] * (num_subtasks - 1) + np.array(subtask_embedding)) / num_subtasks
                ...
                G.add_node(node_counter, subtasks=[subtask], tids=[tid], response = "", execute_losses = [])
                node_to_content_vecs[node_counter] = subtask_embedding
                node_to_rd_vecs[node_counter] = rd_embedding
                G.nodes[node_counter]['role_description'] = role_description
                # system_prompt = "You can answer questions directly or use the appropriate tools. Just give concise answer for the subtask, not the final answer. If the chosen tool is \"browse_url\" and parameter \"start_url\" is not specified, it should be usually \"https://www.google.com/\". The final used url should be saved in response."
                # fixed_system_prompt = f"Input: TOTAL_TASK, CURRENT_STEP. Target: As the role of {role_description} Execute the CURRENT_STEP without extra text, explanation, comment and final answer code."
                ...
                # # system_prompt = "You can answer questions directly or use the appropriate tools. Just give concise answer for the subtask, not the final answer. If the chosen tool is \"browse_url\" and parameter \"start_url\" is not specified, it should be usually \"https://www.google.com/\". The final used url should be saved in response."
                # fixed_system_prompt = f"Input: TOTAL_TASK, CURRENT_STEP. Target: As the role of {role_description} Execute the CURRENT_STEP without extra text, explanation, comment and final answer code. "
                # fixed_system_prompt = tg.Variable(fixed_system_prompt, requires_grad=False, role_description="Structured system prompt to a language model that specifies the behavior and strategies for the QA task. Its content is fixed.")
                # G.nodes[node_counter]['fixed_system_prompt'] = fixed_system_prompt
                # updated_system_prompt = "Operating Principles: (1) Focus only on the current step; do not expand scope, jump ahead or solve the total task by yourself. (2) Never guess; surface uncertainties and risks explicitly."
                # updated_system_prompt = tg.Variable(updated_system_prompt, requires_grad=True, role_description="Structured system prompt to a language model that specifies the behavior and strategies for the QA task. Its content will be updated.")
                # G.nodes[node_counter]['updated_system_prompt'] = updated_system_prompt
                # system_prompt = tg.sum([fixed_system_prompt, updated_system_prompt])
                # G.nodes[node_counter]['executor_model'] = tg.BlackboxLLM(tg.get_engine("gpt-4o-mini"), system_prompt)
                # current_node_path.append(node_counter)
                # node_counter += 1
        ...
        print(current_node_path, file=f, flush=True)
