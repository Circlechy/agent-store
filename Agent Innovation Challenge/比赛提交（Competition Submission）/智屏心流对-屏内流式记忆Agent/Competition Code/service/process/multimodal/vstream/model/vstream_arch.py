...
from service.process.multimodal.vstream.model.multimodal_encoder.builder import build_vision_tower
from service.process.multimodal.vstream.model.multimodal_projector.builder import build_vision_projector
from service.process.multimodal.vstream.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

from service.process.multimodal.vstream.model.compress_functions import drop_feature, merge_feature, kmeans_feature, weighted_kmeans_feature, k_drop_feature, k_merge_feature, attention_feature
...
# 拥有vision_tower和attention_model属性
...
class VStreamMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_streaming(  # Asynchronous encoding with a SemLock, only for videos, batch_size=1
        ...
        while attempt_times < 300:
            try:
                cur_memory, long_memory_compreesed, Turing_memory_compreesed, _ = self.storage.read()
    ...
    def embed_video_streaming(  # Asynchronous encoding with a SemLock, only for videos, batch_size=1
        ...
        return image_features
    def stored_video_streaming(self,image_features):
        logger = logging.getLogger(__name__)
        video_long_memory_length = getattr(self.config, "video_long_memory_length", 10)
        video_Turing_memory_length = getattr(self.config, "video_Turing_memory_length", 10)
        video_short_memory_length = getattr(self.config, "video_short_memory_length", 10)  # not used
        ...
        img_feature_buffer = image_feature.cpu() # 将特征转移到 CPU 上，用于后续的内存管理。
        ...
        # 读取数据（自动加锁）
        video_embedding_memory = self.storage.read()
        if video_embedding_memory is not None and len(video_embedding_memory) > 0:
            old_cur_memory, old_long_memory_compreesed, old_Turing_memory_compreesed, old_img_feature_buffer = video_embedding_memory
            ...
            cur_memory = torch.cat([key_memory, cur_memory], dim=0) #将关键帧添加到 cur_memory 中
            #对合并后的 Turing_memory 进行注意力更新。
        ...
        video_embedding_memory[:] = [cur_memory.cpu(), long_memory_compreesed.cpu(), Turing_memory_compreesed.cpu(), img_feature_buffer]  # Only change content
        # 写入文件（自动加锁）
        self.storage.write(video_embedding_memory)
