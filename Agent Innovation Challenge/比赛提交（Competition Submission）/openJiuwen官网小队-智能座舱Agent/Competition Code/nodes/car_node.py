...
from tools.carTools.get_all_state import get_all_state
from tools.carTools.passenger import (
    get_current_passengers,
    ...
    record_passenger_action,
    get_current_speaker_info,
    identify_speaker_by_seat,
)
    ...
    async def _do_invoke(self, inputs, runtime, context):
        ...
        for i in range(max_tool_loop):
            ...
            if response and response.tool_calls != []:
                ...
                logger.info(f"Car Node决定调用工具: {tool_names}")
        ...
        if plan:
            logger.info(f"Car Node Result: {result}")
