...
## 任务
根据工作流创建指令和工作流描述，以及需要使用的组件信息，生成components.py的代码。

## components.py 的内容
components.py 中实现所有的组件创建函数，每个组件创建函数必须有完整的实现，不能只是函数签名。
...
3. **添加详细的文档字符串**：每个函数都应该有清晰的说明

4. **保持代码简洁**：每个组件只负责一个明确的任务
