...
with open(os.path.join(current_dir_path, "api_mapping_out_excel.json"), "r", encoding="utf8") as f:
...
class TorchToMindSporeTransformer(cst.CSTTransformer):
    ...
    def leave_Call(self, original_node: cst.Call, updated_node: cst.Call) -> cst.Call:
        ...
        if wrapper_pt_full:
            ...
            self.not_converted_api.add(wrapper_pt_full)
        ...
        if isinstance(updated_node.func, cst.Attribute):
            ...
            if attr_name in self.METHOD_ATTR_REMAP:
                ...
                if not api_conf:
                    ...
                    self.not_converted_api.add(pt_ref)
        ...
        if pt_full_name and not api_conf:
            ...
            self.not_converted_api.add(pt_full_name)
    ...
    def _convert_decorator(self, dec: cst.Decorator, notes: List[str]) -> Optional[cst.Decorator]:
        ...
        if not pt_full_name:
            if full_name and full_name.startswith("torch"):
                ...
                self.not_converted_api.add(full_name)
            return dec
...
def convert_code(code: str) -> str:
    ...
    pt2ms_transformers = TorchToMindSporeTransformer(
            API_MAP,
            ms_aliases=None,
    ...
    not_converted_api = pt2ms_transformers.not_converted_api
    ...
    return _ensure_default_ms_imports(new_code),not_converted_api
...
def _convert_and_save(filename: str, code= None, show_diff: bool = True) -> None:
    """
    ...
    result,not_converted_api = convert_code(code)

    return result,not_converted_api

def handler_plugin(target:dict=None):
    in_memory_file = target
    new_outs  ={}
    not_converted_apis = set()
    for file_name in in_memory_file.keys():
        content = in_memory_file[file_name]
        results,not_converted_api = _convert_and_save(filename=file_name,code=content)
        new_outs[file_name] = results
        if len(not_converted_api) > 0:
            for api in not_converted_api:
                not_converted_apis.add(api)
    return new_outs, not_converted_apis
