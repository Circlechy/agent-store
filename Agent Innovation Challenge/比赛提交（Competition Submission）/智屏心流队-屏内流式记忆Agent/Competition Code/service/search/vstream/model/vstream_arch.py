...
from service.search.vstream.model.multimodal_encoder.builder import build_vision_tower
from service.search.vstream.model.multimodal_projector.builder import build_vision_projector
from service.search.vstream.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

from service.search.vstream.model.compress_functions import drop_feature, merge_feature, kmeans_feature, weighted_kmeans_feature, k_drop_feature, k_merge_feature, attention_feature
...
class VStreamMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (features is not None) or (type(images) is list) or (images.ndim == 5):
            ...
            image_features = self.cat_proj(image_features) if image_features else torch.empty()
        else:
    ...
    def prepare_inputs_labels_for_multimodal_streaming(  # Asynchronous encoding with a SemLock, only for videos, batch_size=1
        ...
        image_features = [x.to(self.device) for x in image_features]  # [B, TP, D]
        image_features = self.cat_proj(image_features) if image_features else torch.empty((0,0,0))
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                cur_new_labels.append(cur_labels_noim[i])
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx] if len(image_features) > cur_image_idx else torch.empty((0, 4096), device=cur_labels.device, dtype=cur_labels.dtype)
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
