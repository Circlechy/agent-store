...
from jiuwenfintech.data.cache import get_cache
from jiuwenfintech.data.cache.mongodb_cache_adapter import get_mongodb_cache_adapter
from openjiuwen.core.common.logging import logger

from jiuwenfintech.config.config_manager import config_manager
from jiuwenfintech.config.runtime_settings import get_float, get_timezone_name
...
"""
    ...
    def get_stock_data(self, symbol: str, start_date: str, end_date: str,
        ...
        try:
            ...
            if "" in formatted_data or "错误" in formatted_data:
                ...
                logger.warning(f" [数据来源: 备用数据] 生成备用数据: {symbol}")
    ...
    def get_fundamentals_data(self, symbol: str, force_refresh: bool = False) -> str:
        ...
        try:
            ...
            logger.warning(f" [数据来源: 备用数据] 生成备用基本面数据: {symbol}")
    ...
    def _get_stock_basic_info_only(self, symbol: str) -> str:
        ...
        try:
            ...
            try:
                from jiuwenfintech.config.runtime_settings import use_app_cache_enabled
...
成交量: {volume}"""
            ...
            logger.warning(f" [基本面优化] 获取{symbol}基础信息失败: {e}")
            return f"股票代码: {symbol}\n股票名称: 未知公司\n当前价格: N/A\n涨跌幅: N/A\n成交量: N/A"

    async def _generate_fundamentals_report(self, symbol: str, stock_data: str, analysis_modules: str = "standard") -> str:
        """基于股票数据生成真实的基本面分析报告
        ...
        try:
            financial_estimates = await self._estimate_financial_metrics(symbol, current_price)
            logger.debug(f" [股票代码追踪] _estimate_financial_metrics 返回结果: {financial_estimates}")
        except Exception as e:
            logger.warning(f" [基本面分析] 无法获取财务指标: {e}")
...
"""
    ...
    def _get_industry_info(self, symbol: str) -> dict:
        ...
        try:
            ...
            logger.warning(f" 从数据库获取行业信息失败: {e}")
    ...
    async def _estimate_financial_metrics(self, symbol: str, current_price: str) -> dict:
        ...
        real_metrics = await self._get_real_financial_metrics(symbol, price_value)
        if real_metrics:
    ...
    async def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            if db_manager.is_mongodb_available():
                try:
                    ...
                    if quote and quote.get("close"):
                        ...
                        logger.info(f" market_quotes 中未找到{code6}的实时股价，使用传入价格: {price_value}元")
                except Exception as e:
                    logger.warning(f" 从 market_quotes 获取实时股价失败: {e}，使用传入价格: {price_value}元")
            else:
                logger.info(f" MongoDB 不可用，使用传入价格: {price_value}元")
            ...
            if use_app_cache_enabled(False):
                ...
                if financial_data:
                    ...
                    if metrics:
                        ...
                        logger.warning(f" MongoDB 财务数据解析失败")
                else:
            ...
            if akshare_provider.connected:
                financial_data = await akshare_provider.get_financial_data(symbol)
                ...
                if financial_data and any(not v.empty if hasattr(v, 'empty') else bool(v) for v in financial_data.values()):
                    ...
                    stock_info = await akshare_provider.get_stock_basic_info(symbol)
                    ...
                    if metrics:
                        ...
                        logger.warning(f" AKShare解析失败，返回None")
                else:
                    logger.warning(f" AKShare未获取到{symbol}财务数据，尝试Tushare")
            else:
                logger.warning(f" AKShare未连接，尝试Tushare")
            ...
            await provider.get_financial_data(symbol)
    ...
    def _parse_mongodb_financial_data(self, financial_data: dict, price_value: float) -> dict:
        ...
        try:
            ...
            if roe is not None and str(roe) != 'nan' and roe != '--':
                try:
                    ...
                    if -200 <= roe_val <= 200:
                        ...
                        logger.warning(f" ROE 数据异常: {roe_val}，超出合理范围 [-200%, 200%]，设为 N/A")
                        metrics["roe"] = "N/A"
            ...
            if roa is not None and str(roa) != 'nan' and roa != '--':
                try:
                    ...
                    if -100 <= roa_val <= 100:
                        ...
                        logger.warning(f" ROA 数据异常: {roa_val}，超出合理范围 [-100%, 100%]，设为 N/A")
                        metrics["roa"] = "N/A"
                except (ValueError, TypeError):
                    metrics["roa"] = "N/A"
            else:
            ...
            if gross_margin is not None and str(gross_margin) != 'nan' and gross_margin != '--':
                try:
                    ...
                    if -100 <= gross_margin_val <= 100:
                        ...
                        logger.warning(f" 毛利率数据异常: {gross_margin_val}，超出合理范围 [-100%, 100%]，设为 N/A")
            ...
            if net_margin is not None and str(net_margin) != 'nan' and net_margin != '--':
                try:
                    ...
                    if -100 <= net_margin_val <= 100:
                        ...
                        logger.warning(f" 净利率数据异常: {net_margin_val}，超出合理范围 [-100%, 100%]，设为 N/A")
            ...
            try:
                from jiuwenfintech.data.realtime_metrics import get_pe_pb_with_fallback
                from jiuwenfintech.config.database_manager import get_database_manager
                        ...
                        if realtime_metrics:
                            ...
                            if pe_value is not None and pe_value > 0:
                                ...
                                if pe_ttm_check is None or pe_ttm_check <= 0 or str(pe_ttm_check) == 'nan' or pe_ttm_check == '--':
                                    is_loss_stock = True
                                    logger.info(f" [PE计算-第1层] PE为None且pe_ttm={pe_ttm_check}，确认为亏损股")
                            ...
                            if pe_ttm_value is not None and pe_ttm_value > 0:
                                ...
                                if pe_ttm_check is None or pe_ttm_check <= 0 or str(pe_ttm_check) == 'nan' or pe_ttm_check == '--':
                                    is_loss_stock = True
                                    logger.info(f" [PE_TTM计算-第1层] PE_TTM为None且pe_ttm={pe_ttm_check}，确认为亏损股")
                            ...
                            if pe_ttm_static is None or pe_ttm_static <= 0 or str(pe_ttm_static) == 'nan' or pe_ttm_static == '--':
                                is_loss_stock = True
                                logger.info(f" [PE计算-第1层失败] 检测到亏损股（pe_ttm={pe_ttm_static}），跳过降级计算")
                            else:
                                logger.warning(f" [PE计算-第1层失败] 实时计算返回空结果，将尝试降级计算")

            except Exception as e:
                logger.warning(f" [PE计算-第1层异常] 实时计算失败: {e}，将尝试降级计算")
            ...
            if "total_mv" not in metrics:
                ...
                if total_mv_static is not None and total_mv_static > 0:
                    ...
                    if money_cap is not None and money_cap > 0:
                        ...
                        logger.warning(f" [总市值-全部失败] 无可用总市值数据")

            if pe_value is None:
                if is_loss_stock:
                    metrics["pe"] = "N/A"
                    logger.info(f" [PE计算-亏损股] 已确认为亏损股，PE设置为N/A，跳过第2层计算")
                else:
                    ...
                    if net_profit and net_profit > 0:
                        try:
                            ...
                            if money_cap and money_cap > 0:
                                ...
                                logger.warning(f" [PE计算-第2层失败] 市值无效: {money_cap}，尝试第3层")
                                ...
                                if pe_static is not None and str(pe_static) != 'nan' and pe_static != '--':
                                    try:
                                        ...
                                        if pe_float > 0:
                                            ...
                                            metrics["pe"] = "N/A"
                                            logger.info(f" [PE计算-第3层跳过] 静态PE为负数或零（亏损股）: {pe_float}")
                                    except (ValueError, TypeError):
                                        metrics["pe"] = "N/A"
                        ...
                        metrics["pe"] = "N/A"
                        logger.info(f" [PE计算-亏损股] 净利润为负数（{net_profit}万元），PE设置为N/A")
                    else:
                        logger.warning(f" [PE计算-第2层跳过] 净利润无效: {net_profit}，尝试第3层")
            ...
            if pe_ttm_value is None:
                if is_loss_stock:
                    metrics["pe_ttm"] = "N/A"
                    logger.info(f" [PE_TTM计算-亏损股] 已确认为亏损股，PE_TTM设置为N/A")
                else:
                    ...
                    if pe_ttm_static is not None and str(pe_ttm_static) != 'nan' and pe_ttm_static != '--':
                        try:
                            ...
                            if pe_ttm_float > 0:
                                ...
                                metrics["pe_ttm"] = "N/A"
                                logger.info(f" [PE_TTM计算-第2层跳过] 静态PE_TTM为负数或零（亏损股）: {pe_ttm_float}")
                        except (ValueError, TypeError):
                            metrics["pe_ttm"] = "N/A"
                            logger.error(f" [PE_TTM计算-第2层失败] 静态PE_TTM格式错误: {pe_ttm_static}")
                    else:
                        metrics["pe_ttm"] = "N/A"
                        logger.warning(f" [PE_TTM计算-全部失败] 无可用PE_TTM数据")
        ...
        try:
                ...
                if stock_code:
                    ...
                    if db_manager.is_mongodb_available():
                        ...
                        if realtime_metrics:
                            ...
                            logger.warning(f" [AKShare-PE计算-第1层失败] 实时计算返回空结果，将尝试降级计算")
            except Exception as e:
                logger.warning(f" [AKShare-PE计算-第1层异常] 实时计算失败: {e}，将尝试降级计算")
            ...
            if "total_mv" not in metrics:
                ...
                if total_mv_static is not None and total_mv_static > 0:
                    ...
                    metrics["total_mv"] = "N/A"
                    logger.warning(f" [AKShare-总市值-全部失败] 无可用总市值数据")
            ...
            if pe_value is None:
                ...
                if eps_for_pe and eps_for_pe > 0:
                    ...
                    metrics["pe"] = "N/A（亏损）"
                    logger.warning(f" [AKShare-PE计算-第2层失败] 亏损股票，EPS={eps_for_pe}")
                else:
                    metrics["pe"] = "N/A"
                ...
                if bps_value is not None and str(bps_value) != 'nan' and bps_value != '--':
                    try:
                        ...
                        if bps_val > 0:
                            ...
                            metrics["pb"] = "N/A"
                            logger.warning(f" [AKShare-PB计算-第2层失败] BPS无效: {bps_val}")
            ...
            if revenue_for_ps and revenue_for_ps > 0:
                ...
                if total_share and total_share > 0:
                    ...
                    logger.warning(f" 无法计算PS: 缺少总股本数据")
            else:
                metrics["ps"] = "N/A"
    ...
    def _parse_financial_data(self, financial_data: dict, stock_info: dict, price_value: float) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f" Tushare TTM 计算失败: {e}")
            ...
            if financial_doc:
                ...
                if 'raw_data' in financial_doc and isinstance(financial_doc['raw_data'], dict):
                    ...
                    if 'cashflow_statement' in raw_data and raw_data['cashflow_statement']:
                        financial_data['cash_flow'] = raw_data['cashflow_statement']
                    if 'financial_indicators' in raw_data and raw_data['financial_indicators']:
                        financial_data['main_indicators'] = raw_data['financial_indicators']
                ...
                if financial_data:
                    ...
                    logger.warning(f" [财务数据] {symbol}的 stock_financial_data 记录存在但无有效财务数据字段")
            else:
