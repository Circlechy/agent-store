...
class MongoDBCacheAdapter:
    ...
    def _init_mongodb_connection(self):
        """初始化MongoDB连接"""
        try:
            from jiuwenfintech.config.database_manager import get_mongodb_client
            self.mongodb_client = get_mongodb_client()
            if self.mongodb_client:
                self.db = self.mongodb_client.get_database('tradingagents')
                logger.debug(" MongoDB连接初始化成功")
            else:
                logger.warning(" MongoDB客户端不可用，回退到传统模式")
            ...
            if self.db is not None:
                ...
                if config_data and config_data.get('data_source_configs'):
                    ...
                    for ds in configs:
                        ...
                        if not ds_enabled:
                            logger.info(f" [数据源优先级] {ds_type} 未启用，跳过")
                            continue

                        if ds_categories and market_category:
                            if market_category not in ds_categories:
                                logger.info(f" [数据源优先级] {ds_type} 不支持市场 {market_category}，跳过")
                                continue
                    ...
                    if result:
                        ...
                        logger.warning(f" [数据源优先级] 没有可用的数据源配置，使用默认顺序")
                else:
                    logger.warning(f" [数据源优先级] 数据库中没有找到数据源配置")
        ...
        try:
            ...
            for data_source in priority_order:
                ...
                if data:
                    ...
                    logger.debug(f" [MongoDB-{data_source}] 未找到{period}数据: {symbol}")

            logger.warning(f" [数据来源: MongoDB] 所有数据源({', '.join(priority_order)})都没有{period}数据: {symbol}，降级到其他数据源")
            return None
    ...
    def get_financial_data(self, symbol: str, report_period: str = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f" [数据来源: MongoDB-财务数据] 获取财务数据失败: {e}")
            return None
    ...
    def get_news_data(self, symbol: str = None, hours_back: int = 24, limit: int = 20) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            ...
            logger.warning(f" [数据来源: MongoDB-新闻数据] 获取新闻数据失败: {e}")
            return None
