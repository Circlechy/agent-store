...
def calculate_realtime_pe_pb(
    ...
    try:
        ...
        if not quote:
            logger.warning(f" [实时PE计算-失败] 未找到股票 {code6} 的实时行情数据")
            return None
        ...
        if not realtime_price or realtime_price <= 0:
            logger.warning(f" [实时PE计算-失败] 股票 {code6} 的实时价格无效: {realtime_price}")
            return None
        ...
        total_mv_yi = basic_info.get("total_mv")
        total_share = basic_info.get("total_share")
        basic_info_updated_at = basic_info.get("updated_at")
        ...
        if total_share and total_share > 0:
            ...
            if pre_close and pre_close > 0:
                ...
                logger.info(f"    market_quotes 中无 pre_close，使用 stock_basic_info 市值作为昨日市值: {yesterday_mv_yi:.2f}亿元")
            else:
                logger.warning(f" [动态PE计算-失败] 无法获取昨日市值: pre_close={pre_close}, total_mv={total_mv_yi}")
                return None
            ...
            total_shares_wan = (total_mv_yi * 10000) / realtime_price
            yesterday_mv_yi = total_mv_yi
            logger.warning(f"    market_quotes 中无 pre_close，假设 stock_basic_info.total_mv 是昨日市值")
            ...
            logger.warning(f" [动态PE计算-失败] 无法获取总股本数据")
        ...
        if not pe_ttm_tushare or pe_ttm_tushare <= 0 or not yesterday_mv_yi or yesterday_mv_yi <= 0:
            logger.warning(f" [动态PE计算-失败] 无法反推TTM净利润: pe_ttm={pe_ttm_tushare}, yesterday_mv={yesterday_mv_yi}")
        ...
        if financial_data:
            ...
            if total_equity and total_equity > 0:
                ...
                logger.warning(f"    PB计算失败: 净资产无效 ({total_equity})")
        else:
            logger.warning(f"    未找到财务数据，无法计算PB")
            if pb_tushare:
                pb = pb_tushare
...
def get_pe_pb_with_fallback(
    ...
    if realtime_metrics:
        ...
        if validate_pe_pb(pe, pb):
            ...
            logger.warning(f" [PE智能策略-方案1异常] 动态PE/PB超出合理范围 (PE={pe}, PB={pb})")
    ...
    try:
        ...
        logger.warning(" [PE智能策略-方案2失败] Tushare静态数据不可用")

    except Exception as e:
        logger.warning(f" [PE智能策略-方案2异常] {e}")
