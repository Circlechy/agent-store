...
class BaoStockProvider(BaseStockDataProvider):
        ...
        try:
            ...
            for row in data_list:
                if len(row) >= 6:
                    code = row[0]
                    name = row[1]
                    stock_type = row[4] if len(row) > 4 else '0'
                    status = row[5] if len(row) > 5 else '0'
    ...
    async def get_historical_data(self, code: str, start_date: str, end_date: str,
        ...
        try:
            ...
            if not data_list:
                logger.warning(f" BaoStock历史数据为空: {code}")
                return None
    ...
    async def get_financial_data(self, code: str, year: Optional[int] = None,
        ...
        try:
            ...
            if financial_data:
                ...
                logger.warning(f" BaoStock财务数据为空: {code}")
