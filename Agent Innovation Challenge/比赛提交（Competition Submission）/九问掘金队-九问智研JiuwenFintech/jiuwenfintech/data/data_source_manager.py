...
from jiuwenfintech.constants import DataSourceCode
...
class DataSourceManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(f" 统一缓存管理器初始化失败: {e}")
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                if result:
                    ...
                    logger.warning(f" [数据源优先级] 市场={market_category or '全部'}, 数据库配置中没有可用的数据源，使用默认顺序")
            else:
                logger.warning(" [数据源优先级] 数据库中没有数据源配置，使用默认顺序")
        except Exception as e:
            logger.warning(f" [数据源优先级] 从数据库读取失败: {e}，使用默认顺序")
    ...
    def _identify_market_category(self, symbol: Optional[str]) -> Optional[str]:
        ...
        try:
            from jiuwenfintech.utils.stock_utils import StockUtils, StockMarket
    ...
    def get_fundamentals_data(self, symbol: str) -> str:
        ...
        try:
            ...
            if result and "" not in result:
                ...
                logger.warning(f" [数据来源: {self.current_source.value}失败] 基本面数据质量异常，尝试降级: {symbol}",
                              extra={
                                  'symbol': symbol,
                                  'data_source': self.current_source.value,
    ...
    def get_news_data(self, symbol: str = None, hours_back: int = 24, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        try:
            if self.current_source == ChinaDataSource.MONGODB:
                ...
                logger.warning(f" 数据源 {self.current_source.value} 不支持新闻数据")
                result = []
            ...
            if result and result_count > 0:
                ...
                logger.warning(f" [数据来源: {self.current_source.value}] 未获取到新闻数据: {symbol or '市场新闻'}，尝试降级",
                              extra={
                                  'symbol': symbol,
                                  'data_source': self.current_source.value,
                                  'duration': duration,
    ...
    def _check_available_sources(self) -> List[ChinaDataSource]:
        ...
        try:
            ...
            if config_data and config_data.get('data_source_configs'):
                ...
                logger.warning(" [数据源配置] 数据库中没有数据源配置，将检查所有已安装的数据源")
                enabled_sources_in_db = {'mongodb', 'tushare', 'akshare', 'baostock'}
        except Exception as e:
            logger.warning(f" [数据源配置] 从数据库读取失败: {e}，将检查所有已安装的数据源")
            enabled_sources_in_db = {'mongodb', 'tushare', 'akshare', 'baostock'}
        ...
        if self.use_mongodb_cache and 'mongodb' in enabled_sources_in_db:
            try:
                ...
                if adapter.use_app_cache and adapter.db is not None:
                    ...
                    logger.warning(" MongoDB数据源不可用: 数据库未连接")
            except Exception as e:
                logger.warning(f" MongoDB数据源不可用: {e}")
        ...
        if 'tushare' in enabled_sources_in_db:
            try:
                ...
                if token:
                    ...
                    logger.warning(" Tushare数据源不可用: API Key未配置（数据库和环境变量均未找到）")
            except ImportError:
    ...
    def _get_datasource_configs_from_db(self) -> dict:
        ...
        try:
            ...
            logger.warning(f" 从数据库读取数据源配置失败: {e}")
            return {}
        ...
        try:
            ...
            if len(data) > 1:
                ...
                if prev_dif <= prev_dea and curr_dif > curr_dea:
                    result += "    MACD金叉信号（DIF上穿DEA）\n\n"
                elif prev_dif >= prev_dea and curr_dif < curr_dea:
                    result += "    MACD死叉信号（DIF下穿DEA）\n\n"
                else:
                    result += "\n"
            else:
                result += "\n"
    ...
    def get_stock_dataframe(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(f" [DataFrame接口] {self.current_source.value} 失败，尝试降级")
            for source in self.available_sources:
                ...
                try:
                    ...
                    logger.warning(f" [DataFrame接口] {source.value} 失败: {e}")
                    continue
    ...
    async def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        """
        获取股票数据的统一接口，支持多周期数据
        ...
        try:
            ...
            if self.current_source == ChinaDataSource.MONGODB:
                ...
                result = await self._get_akshare_data(symbol, start_date, end_date, period)
            ...
            duration = time.time() - start_time
            result_length = len(result) if result else 0
            is_success = result and "错误" not in result
            ...
            if is_success:
                ...
                logger.warning(f" [数据来源: {self.current_source.value}失败] 数据质量异常，尝试降级到其他数据源: {symbol}",
                              extra={
                                  'symbol': symbol,
                                  'start_date': start_date,
                                  'end_date': end_date,
    ...
    def _get_tushare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                duration = time.time() - start_time
                logger.warning(f" [Tushare] 未获取到数据，耗时={duration:.2f}s")
                return result
        except Exception as e:
            duration = time.time() - start_time
    ...
    async def _get_akshare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
            ...
            if source != self.current_source and source in self.available_sources:
                try:
                    ...
                    if "" not in result:
                        ...
                        logger.warning(f" [备用数据源-{source.value}] 返回错误结果: {symbol}")
            ...
            try:
                ...
                if doc:
                    ...
                    if name:
                        ...
                        logger.warning(f" [数据来源: MongoDB] 未找到有效名称: {symbol}，降级到其他数据源")
            except Exception as e:
        ...
        try:
            if self.current_source == ChinaDataSource.TUSHARE:
                ...
                if result.get('name') and result['name'] != f'股票{symbol}':
                    ...
                    logger.warning(f" [数据来源: Tushare失败] 返回无效信息，尝试降级: {symbol}")
                ...
                if adapter and hasattr(adapter, 'get_stock_info'):
                    ...
                    if result.get('name') and result['name'] != f'股票{symbol}':
                        ...
                        logger.warning(f" [数据来源: {self.current_source.value}失败] 返回无效信息，尝试降级: {symbol}")
                        return self._try_fallback_stock_info(symbol)
                else:
                    logger.warning(f" [数据来源: {self.current_source.value}] 不支持股票信息获取，尝试降级: {symbol}")
                    return self._try_fallback_stock_info(symbol)
    ...
    def get_stock_basic_info(self, stock_code: str = None) -> Optional[Dict[str, Any]]:
        ...
        if stock_code is None:
            ...
            try:
                ...
                logger.warning(f" 从MongoDB获取所有股票失败: {e}")
        ...
        for source_name in available_sources:
            try:
                ...
                if result.get('name') and result['name'] != f'股票{symbol}':
                    ...
                    logger.warning(f" [数据来源: {source_name}] 返回无效信息")
        ...
        try:
            ...
            if stock_info is not None and not stock_info.empty:
                ...
                if not name_row.empty:
                    ...
                    logger.warning(f" [AKShare股票信息] 未找到股票简称: {symbol}")
                ...
                logger.warning(f" [AKShare股票信息] 返回空数据: {symbol}")
            ...
            if financial_data is not None:
                if isinstance(financial_data, pd.DataFrame):
                    if not financial_data.empty:
                        ...
                        logger.warning(f" [数据来源: MongoDB] 财务数据为空: {symbol}，降级到其他数据源")
                        return self._try_fallback_fundamentals(symbol)
                    ...
                    logger.warning(f" [数据来源: MongoDB] 未找到财务数据: {symbol}，降级到其他数据源")
                    return self._try_fallback_fundamentals(symbol)
            else:
                logger.warning(f" [数据来源: MongoDB] 未找到财务数据: {symbol}，降级到其他数据源")
                return self._try_fallback_fundamentals(symbol)
    ...
    def _get_tushare_fundamentals(self, symbol: str) -> str:
        ...
        logger.warning(f" Tushare基本面数据功能暂时不可用")
    ...
    def _try_fallback_fundamentals(self, symbol: str) -> str:
        ...
        logger.warning(f" [数据来源: 生成分析] 所有数据源失败，生成基本分析: {symbol}")
        return self._generate_fundamentals_analysis(symbol)
    ...
    def _get_mongodb_news(self, symbol: str, hours_back: int, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if news_data and len(news_data) > 0:
                ...
                logger.warning(f" [数据来源: MongoDB] 未找到新闻: {symbol or '市场新闻'}，降级到其他数据源")
                return self._try_fallback_news(symbol, hours_back, limit)
    ...
    def _get_tushare_news(self, symbol: str, hours_back: int, limit: int) -> List[Dict[str, Any]]:
        """从Tushare获取新闻数据"""
        try:
            logger.warning(f" [数据来源: Tushare] Tushare新闻功能暂时不可用")
            return []
    ...
    def _get_akshare_news(self, symbol: str, hours_back: int, limit: int) -> List[Dict[str, Any]]:
        """从AKShare获取新闻数据"""
        try:
            logger.warning(f" [数据来源: AKShare] AKShare新闻功能暂时不可用")
            return []
            ...
            if source != self.current_source and source in self.available_sources:
                try:
                    ...
                    if result and len(result) > 0:
                        ...
                        logger.warning(f" 备用数据源{source.value}未返回新闻")

                except Exception as e:
                    logger.error(f" 备用数据源{source.value}异常: {e}")
                    continue

        logger.warning(f" [数据来源: 所有数据源失败] 无法获取新闻: {symbol or '市场新闻'}")
        return []
...
async def get_china_stock_data_unified(symbol: str, start_date: str, end_date: str) -> str:
    """
    统一的中国股票数据获取接口
    自动使用配置的数据源，支持备用数据源
    ...
    result = await manager.get_stock_data(symbol, start_date, end_date)
    if result:
        lines = result.split('\n')
...
def get_stock_data_service() -> DataSourceManager:
    ...
    获取股票数据服务实例（兼容 stock_data_service 接口）
     ...
     此函数为兼容性接口，实际返回 DataSourceManager 实例
...
class USDataSourceManager:
    ...
    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[USDataSource]:
        ...
        try:
            ...
            logger.warning(" [美股数据源优先级] 数据库中没有配置，使用默认顺序")
        except Exception as e:
            logger.warning(f" [美股数据源优先级] 从数据库读取失败: {e}，使用默认顺序")
    ...
    def _check_available_sources(self) -> List[USDataSource]:
        ...
        if 'yfinance' in enabled_sources_in_db:
            try:
                ...
                logger.warning(" yfinance数据源不可用: 未安装 yfinance 库")
        else:
        ...
        if 'alpha_vantage' in enabled_sources_in_db:
            try:
                ...
                if api_key:
                    ...
                    logger.warning(" Alpha Vantage数据源不可用: API Key未配置（数据库和环境变量均未找到）")
            except Exception as e:
                logger.warning(f" Alpha Vantage数据源检查失败: {e}")
        else:
        ...
        if 'finnhub' in enabled_sources_in_db:
            try:
                ...
                if api_key:
                    ...
                    logger.warning(" Finnhub数据源不可用: API Key未配置（数据库和环境变量均未找到）")
            except Exception as e:
                logger.warning(f" Finnhub数据源检查失败: {e}")
        else:
    ...
    def _get_enabled_sources_from_db(self) -> List[str]:
        ...
        try:
            ...
            logger.warning(f" 从数据库读取启用的数据源失败: {e}")
